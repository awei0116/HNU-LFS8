...
if True:
    ...
    class CrossScan_multimodal(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x_e: torch.Tensor, x_lf: torch.Tensor):
            # B, C, H, W -> B, 3, C, 2 * H * W
            B, C, H, W = x_lf.shape
            ctx.shape = (B, C, H, W)
            xs_fuse = x_lf.new_empty((B, 2, C, 2 * H * W))
            xs_fuse[:, 0] = torch.cat([x_e.flatten(2, 3), x_lf.flatten(2, 3)], dim=2)
    ...
    def cross_selective_scan_multimodal_k2(
        ...
        x_fuse = CrossScan_multimodal.apply(x_e, x_lf) # B, C, H, W -> B, 3, C, 3 * H * W
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", x_fuse, x_proj_weight)
...
class LFConMB_SS2D(nn.Module):
    ...
    def forward_corev2_multimodal(self,x_e: torch.Tensor, x_lf: torch.Tensor, nrows=-1):
        return cross_selective_scan_multimodal_k2(
            x_e, x_lf, self.x_proj_weight, None, self.dt_projs_weight, self.dt_projs_bias,
            self.A_logs, self.Ds, getattr(self, "out_norm1", None), getattr(self, "out_norm2", None), self.softmax_version, 
            nrows=nrows,
        )

    def forward(self, x_e: torch.Tensor, x_lf: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            y_e, y_lf = self.forward_corev2_multimodal(x_e_conv, x_lf_conv) # b, d, h, w -> b, h, w, d
            # SE to get attention, scale
            b, d, h, w = x_lf_trans.shape
            ...
            x_lf_exitation = self.fc1(x_lf_squeeze).view(b, d, 1, 1).permute(0, 2, 3, 1).contiguous()
            y_e = y_e * x_lf_exitation
            y_lf = y_lf * x_e_exitation
            y = torch.concat([y_e, y_lf], dim=-1)
...
class LF_Cross_Mamba_Attention_SSM(nn.Module):
    ...
    def forward(self, x_e: torch.Tensor, x_lf: torch.Tensor):
        ...
        x_dbl_e = self.x_proj_1(rearrange(x_e, "b d l -> (b l) d"))  # (bl d)
        x_dbl_lf = self.x_proj_2(rearrange(x_lf, "b d l -> (b l) d"))  # (bl d)
        ...
        y_e = selective_scan(
            x_e, dt_e,
            A_e, B_e, C_lf, self.D_2.float(),
            delta_bias=self.dt_proj_2.bias.float(),
            delta_softplus=True,
        )
        y_lf = selective_scan(
            x_lf, dt_lf,
            A_lf, B_lf, C_e, self.D_1.float(),
...
class LFCrossMambaFusion_SS2D_SSM(nn.Module):
    ...
    def forward(self, x_e: torch.Tensor, x_lf: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            y_e, y_lf = self.CMA_ssm(x_e_conv, x_lf_conv)
            # to b, d, h, w
...
class LFCrossMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_e: torch.Tensor, x_lf: torch.Tensor):
        x_e_cross, x_lf_cross = self.op(x_e, x_lf)
        x_e = x_e + self.drop_path1(x_e_cross)
        x_lf = x_lf + self.drop_path2(x_lf_cross)
    ...
    def forward(self, x_e: torch.Tensor, x_lf: torch.Tensor):
        ...
        '''
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, x_e, x_lf)
        else:
            return self._forward(x_e, x_lf)
...
class LFConcatMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_e: torch.Tensor, x_lf: torch.Tensor):
        x = x_e + x_lf + self.drop_path(self.op(x_e, x_lf))
...
class Backbone_VSSM_LF(VSSM):
    def __init__(self, patch_size=4, in_chans=51, num_classes=1000,
                 depths=[2, 2, 9, 2], dims=[96, 192, 384, 768],
                 d_state=16, ssm_ratio=2.0, attn_drop_rate=0.,
                 drop_rate=0., drop_path_rate=0.1, mlp_ratio=4.0,
                 patch_norm=True, norm_layer=nn.LayerNorm,
